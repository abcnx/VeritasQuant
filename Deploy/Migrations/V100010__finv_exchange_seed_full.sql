-- =====================================================================
-- FinvQuant PostgreSQL V100010：finv_exchange 全量初始数据（44 条，替换 V100000 存量 22 条）
--
-- 决策（ACANX 2026-08-05）：
--   - 用户提供全量 44 条数据（含 V100000 原有 22 条 + 新增 22 条），
--     作为 finv_exchange 的最终初始数据；
--   - V100000 已发布不可修改，新增 V100010 迁移：先 DELETE 清空存量，
--     再 INSERT 全量 44 条（满足"存量先删除、全量入库"要求）；
--   - 新增 22 条：22 HKFE / 36 NYMEX / 37 CME / 38 CBOT / 40 CA /
--     56 ASX / 57 KRX / 58 BMD / 120 BD / 121 BMS / 122 FD /
--     9001 FTSP / 9002 FTSN / 9003 TFD / 10000 CC / 10001 CRYPTO /
--     10002 CCBA / 10003 CCCOIN / 10004 CCBGO / 10005 CCDDEX /
--     10006 CCHSK / 10007 CCPT；
--   - exchange_code 段位：1~999 常规市场，9001+ 结构化/测试，10000+ 加密货币；
--   - gmt_create / gmt_update 由 DEFAULT now() 生成（不随数据插入），
--     ft_list_exchange_code 留空（映射预留）；
--   - 单事务、失败回滚；重复执行结果一致（先清后插，幂等）。
-- =====================================================================

BEGIN;

-- ---------------------------------------------------------------------
-- 1. 清空存量初始数据（V100000 及手动补充数据）
-- ---------------------------------------------------------------------
