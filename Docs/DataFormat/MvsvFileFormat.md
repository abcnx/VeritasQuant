...
| 行尾空列 | 布局 B 允许末尾 `pc` 为空（如 `...\|4332.1\|` 结尾多一个 `\|`）；布局 A 不允许多余列 | 布局 B 可选 |
...
| 5 | 布局 B 的 `pc` 列可留空（行尾多一个 `\|`） | 与现网 GCmain 文件一致 |
| 6 | 使用 `\n` 换行 | `\r\n` 也兼容，但统一更佳 |
