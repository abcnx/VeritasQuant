...
- **请求**：`GET /API/V1/Meta/Exchange/List`，Query 参数如下：
...
- **请求**：`POST /API/V1/Meta/Exchange/Save`，JSON body 字段见接口文档。
- 新增：`exchange_code` 不存在时 INSERT，`flag_enable` 默认 `'1'`。
- 修改：`exchange_code` 已存在时 UPDATE（主键不可改）。

### 4.3 禁用/启用

- **请求**：`POST /API/V1/Meta/Exchange/Toggle`，body：`{ "exchange_code": 11, "flag_enable": "0" }`。
- 成功后行内状态即时刷新并提示。
