...
- **请求**：`GET /API/V1/Meta/Security/List`，Query 参数如下：
...
- **请求**：`POST /API/V1/Meta/Security/Save`，JSON body 字段见接口文档。
- 新增：`usc` 不存在时 INSERT，`flag_enable` 默认 `'1'`。
- 修改：`usc` 已存在时 UPDATE（主键不可改）。

### 4.3 禁用/启用

- **请求**：`POST /API/V1/Meta/Security/Toggle`，body：`{ "usc": "NVDA", "flag_enable": "0" }`。
...
- 「历史行情查询」菜单的**证券代码筛选下拉字典**来源于本字典：`GET /API/V1/Meta/Security/Options` 返回启用状态证券的 `usc`（key）与 `security_name_cn`（展示值），按 `usc:security_name_cn` 格式展示，保留手动输入。
