      ...
      <v-card>
        ...
        <v-card-text>
          ...
          <v-select v-model="tmplForm.template_type" :items="[
            { title: '策略模板', value: 'STRATEGY' }, { title: '账户模板', value: 'ACCOUNT' }, { title: '环境模板', value: 'ENVIRONMENT' },
          ]" label="模板类型" />
          <v-text-field v-model="tmplForm.description" label="说明" />
          <v-textarea v-model="tmplContentText" label="模板内容（JSON）" rows="14" style="font-family: monospace" spellcheck="false" />
        </v-card-text>
        <v-card-actions>
          <v-spacer />
