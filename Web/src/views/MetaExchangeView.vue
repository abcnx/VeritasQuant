...
async function load() {
  ...
  try {
    const data = await apiGet<{ total: number; list: ExchangeRow[] }>(
      `/Meta/Exchange/List?page=${page.value}&page_size=${pageSize.value}&keyword=${encodeURIComponent(keyword.value)}`,
    )
...
async function save() {
  ...
  try {
    await apiPost('/Meta/Exchange/Save', {
...
async function toggle(row: ExchangeRow) {
  ...
  try {
    await apiPost('/Meta/Exchange/Toggle', { exchange_code: row.exchange_code, flag_enable: next })
