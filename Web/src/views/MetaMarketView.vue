...
async function load() {
  ...
  try {
    const data = await apiGet<{ total: number; list: MarketRow[] }>(
      `/Meta/Market/List?page=${page.value}&page_size=${pageSize.value}&keyword=${encodeURIComponent(keyword.value)}`,
    )
...
async function save() {
  ...
  try {
    await apiPost('/Meta/Market/Save', {
...
async function toggle(row: MarketRow) {
  ...
  try {
    await apiPost('/Meta/Market/Toggle', { market_code: row.market_code, flag_enable: next })
